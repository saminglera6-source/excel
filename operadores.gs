...
// ------------------------------------------------------------
//  EDICIÓN RÁPIDA de datos de contacto — Mis Operaciones. Pensada para
//  arreglar un dato mal cargado (nombre, CUIL, teléfono, domicilio, email)
//  SIN pasar por el flujo completo de "Corregir" (que exige motivo, crea
//  una operación nueva y reemplaza la anterior en toda la cadena de
//  auditoría) — acá solo se pisa el campo puntual, en el momento, para que
//  el próximo recibo salga bien. Por eso mismo la whitelist es angosta:
//  SOLO campos de contacto que no participan de ningún cálculo de
//  caja/ganancia/stock — nunca precios, cantidades, IMEIs de stock, ni
...
function editarDatosContactoOperacion(d) {
  const numero   = String((d && d.numero)   || "").trim();
  const operador = (d && d.operador) || "";
  const campos   = (d && d.campos) || {};
  if (!numero) throw new Error("❌ Falta el número de operación.");
  if (!operador) throw new Error("❌ Falta el operador.");
  if (Object.keys(campos).length === 0) throw new Error("❌ No hay ningún campo para editar.");

  const prefijo = numero.split("-")[0].toUpperCase();
  const permitidos = CAMPOS_EDITABLES_RAPIDO_POR_PREFIJO[prefijo];
  if (!permitidos) throw new Error(`❌ La edición rápida no está disponible para operaciones "${prefijo}".`);
  ...
  const cambios = [];
  permitidos.forEach(nombreCampo => {
    if (!(nombreCampo in campos)) return;
    const col = asegurarColumnaGenerica_(sheet, fE, nombreCampo);
    const valorAnterior = String(sheet.getRange(fila, col).getValue() || "").trim();
    const valorNuevo = String(campos[nombreCampo] || "").trim();
    if (valorAnterior === valorNuevo) return;
    sheet.getRange(fila, col).setValue(valorNuevo);
    cambios.push(`${nombreCampo}: "${valorAnterior || "—"}" → "${valorNuevo || "—"}"`);
  });

  if (cambios.length === 0) return `ℹ️ No hubo cambios para aplicar en "${numero}".`;

  registrarAuditoria_(info.tipo, numero, "EDICION_RAPIDA_DATOS_CONTACTO",
    `${cambios.join(" | ")} (editado por: ${operador}).`);
